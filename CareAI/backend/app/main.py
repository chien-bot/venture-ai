"""FastAPI entry point for the CareAI health-risk MVP."""

from fastapi import FastAPI, HTTPException, Response, status
from fastapi.middleware.cors import CORSMiddleware

from app.database import (
    confirm_weekly_plan_draft, delete_health_report, get_action_plan_for_report, get_health_report, get_health_trends,
    get_user_profile, get_weekly_review, list_health_reports, list_user_profiles,
    save_health_report, save_user_profile, save_weekly_plan_draft, update_plan_task, update_weekly_plan_draft_task,
)
...
from app.services.health_report_agent import HealthReportAgentError, generate_health_report
...
from app.services.risk_config import get_rule_information
...
from app.services.weekly_plan_agent import generate_weekly_plan

app = FastAPI(
    title="CareAI Health Risk Assistant API",
    version="0.1.0",
    description="AI-assisted health-risk analysis only. Not a disease diagnosis or a replacement for clinicians.",
)
...
app.add_middleware(
    ...
    expose_headers=["X-CareAI-Report-Id"],
)
...
async def analyze_health(data: HealthDataInput, response: Response) -> HealthReport:
    ...
    try:
        ...
        saved_report = save_health_report(data, assessment, report)
...
async def create_weekly_plan_draft(user_id: str = "demo-user") -> WeeklyPlanDraft:
    ...
    try:
        ...
        raise HTTPException(status_code=status.HTTP_503_SERVICE_UNAVAILABLE, detail=str(exc)) from exc
    return save_weekly_plan_draft(user_id, profile.health_goal or "建立可持续的健康习惯", content.summary, content.adjustment_reason, content.action_plan)
