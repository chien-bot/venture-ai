...
export function HealthForm({ onSubmit, isLoading, error, userId, defaultGoal }: FormProps) {
  ...
  return (
    <main className="mx-auto max-w-3xl px-5 py-10">
      ...
      <section className="mb-6 rounded-2xl border border-cyan-100 bg-cyan-50/60 p-4"><div className="flex flex-wrap items-baseline justify-between gap-2"><h2 className="font-bold text-cyan-950">快速演示案例</h2><span className="text-xs text-cyan-800">仅用于产品演示，可随时修改数据</span></div><div className="mt-3 grid gap-3 sm:grid-cols-3">{demoScenarios.map((scenario) => <button type="button" key={scenario.title} onClick={() => setForm(scenario.input)} className={`rounded-xl border p-3 text-left transition hover:-translate-y-0.5 hover:shadow-sm ${scenario.tone}`}><p className="text-sm font-bold">{scenario.title}</p><p className="mt-1 text-xs leading-5 opacity-75">{scenario.description}</p><span className="mt-3 inline-block text-xs font-semibold">载入此数据 →</span></button>)}</div></section>
      <section className="mb-6 rounded-2xl border border-amber-100 bg-amber-50/60 p-4"><div className="flex flex-wrap items-baseline justify-between gap-2"><div><h2 className="font-bold text-amber-950">从健康报告建立记录</h2><p className="mt-1 text-xs leading-5 text-amber-900/75">粘贴电子报告文字或选择 TXT 后，系统提取草稿；必须由你在下方确认、编辑后才会保存。</p></div><button type="button" onClick={() => { setReportText(sampleReportText); setReportFilename("课堂样例健康报告.txt"); setImportNotices([]); }} className="text-xs font-semibold text-amber-800 underline underline-offset-4">载入课堂样例</button></div><div className="mt-3 grid gap-3 sm:grid-cols-[1fr_auto]"><textarea value={reportText} onChange={(event) => { setReportText(event.target.value); setImportNotices([]); }} rows={4} placeholder="例如：报告日期：2026-08-20；身高：165 cm；体重：62 kg；血压：128/82" className={inputClass} /><div className="flex flex-col gap-2"><label className="cursor-pointer rounded-xl border border-amber-200 bg-white px-3 py-2 text-center text-xs font-semibold text-amber-800 hover:bg-amber-50">选择 TXT<input type="file" accept=".txt,text/plain" className="hidden" onChange={(event) => void readTextReport(event.target.files?.[0])} /></label><button type="button" disabled={isImporting} onClick={() => void extractReportText()} className="rounded-xl bg-amber-700 px-4 py-2 text-sm font-semibold text-white hover:bg-amber-800 disabled:opacity-60">{isImporting ? "正在提取" : "提取并核对"}</button></div></div>{reportFilename && <p className="mt-2 text-xs text-amber-800">当前文字来源：{reportFilename}</p>}{importNotices.length > 0 && <ul className="mt-3 space-y-1 rounded-xl bg-white/70 p-3 text-xs leading-5 text-amber-950/75">{importNotices.map((notice) => <li key={notice}>• {notice}</li>)}</ul>}</section>
      <form onSubmit={submit} className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm sm:p-8">
        ...
        <div className="mt-7 flex flex-col gap-3 border-t border-slate-100 pt-6 sm:flex-row sm:items-center sm:justify-between"><p className="max-w-md text-xs leading-5 text-slate-500">提交后先由本地规则处理；是否调用受限AI、是否保存报告及保留期限，以“隐私中心”的当前设置为准。本服务不诊断疾病，也不替代医生。</p><button disabled={isLoading} className="inline-flex shrink-0 items-center justify-center gap-2 rounded-xl bg-teal-600 px-5 py-3 font-semibold text-white shadow-lg shadow-teal-600/20 transition hover:bg-teal-700 disabled:cursor-not-allowed disabled:opacity-60">{isLoading ? <><LoaderCircle size={18} className="animate-spin" />正在分析</> : <>生成健康报告 <ArrowRight size={18} /></>}</button></div>
      </form>
    </main>
  );
}
